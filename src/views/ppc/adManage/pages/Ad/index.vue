    ...
    <el-table-column prop="qualification" label="投放资格" width="130" fixed>
      <template slot-scope="{row}">
        ...
        <el-tooltip v-if="row.qualificationMessage" effect="dark" placement="top">
          <div slot="content" class="popper-qualificationMessage">{{ row.qualificationMessage }}</div>
          <div>
            <div slot="reference" class="qualificationMessage">{{ row.qualificationMessage }}</div>
          </div>
        </el-tooltip>
      </template>
    </el-table-column>
